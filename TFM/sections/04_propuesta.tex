...
Las restricciones se comprueban antes de ejecutar una compra simulada. Su finalidad es impedir que una recompensa positiva por el retorno previsto apruebe una operación que no respeta los límites de la cartera. La configuración inicial incluye los siguientes límites, que pueden modificarse en cada experimento:

\begin{itemize}
  \item \textbf{Tamaño de posición:} el importe de una compra no puede superar el 20\,\% del valor neto de la cartera.
  ...
  \item \textbf{Capital bloqueado:} el capital que no puede reutilizarse después de la compra no puede superar el 60\,\% del valor neto.
  ...
  \item \textbf{Disponibilidad y volatilidad:} cuando esos datos están disponibles, la propuesta debe cumplir la cantidad mínima configurada y, si se establece, el límite máximo de volatilidad.
\end{itemize}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[capa] (adquisicion) {\textbf{1. Adquisición}\\{\footnotesize Steam Community Market, \acroref{BUFF}{BUFF}, SteamDT, \acroref{OCR}{OCR} e importaciones manuales generan observaciones.}};
    \node[capa, below=of adquisicion] (persistencia) {\textbf{2. Persistencia}\\{\footnotesize Las observaciones normalizadas, los históricos, las predicciones y las decisiones simuladas quedan registrados.}};
    \node[capa, below=of persistencia] (prediccion) {\textbf{3. Predicción}\\{\footnotesize El modelo supervisado estima la probabilidad de que un candidato cumpla el criterio económico definido.}};
    \node[capa, below=of prediccion] (decision) {\textbf{4. Decisión MARL}\\{\footnotesize Scout, Trader y Portfolio combinan sus observaciones para proponer y validar una acción.}};
    \node[capa, below=of decision] (simulacion) {\textbf{5. Simulación de cartera}\\{\footnotesize La acción aprobada actualiza la cartera simulada y produce métricas, restricciones y recompensas.}};

    \draw[flujo] (adquisicion) -- node[right, font=\scriptsize] {datos normalizados} (persistencia);
    \draw[flujo] (persistencia) -- node[right, font=\scriptsize] {histórico disponible} (prediccion);
    \draw[flujo] (prediccion) -- node[right, font=\scriptsize] {señal estimada} (decision);
    \draw[flujo] (decision) -- node[right, font=\scriptsize] {acción coordinada} (simulacion);
  \end{tikzpicture}
  \caption{Flujo vertical de la arquitectura propuesta.}
