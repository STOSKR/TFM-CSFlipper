...
La arquitectura distingue entre componentes de adquisición y agentes \acroref{MARL}{MARL}. Los componentes de adquisición no se entrenan mediante aprendizaje por refuerzo ni deciden operaciones. Su trabajo es obtener datos desde Steam, \acroref{BUFF}{BUFF}, SteamDT, \acroref{OCR}{OCR} o \acroref{CSV}{CSV}/\acroref{API}{API} y transformarlos en observaciones persistidas. Los agentes \acroref{MARL}{MARL}, en cambio, operan dentro del entorno de simulación.

\begin{table}[h]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.22\textwidth} >{\raggedright\arraybackslash}X}
\toprule
Componente o agente & Función \\
\midrule
\multicolumn{2}{l}{\textit{Adquisición de datos}} \\
Steam y \acroref{BUFF}{BUFF} & Aportan precios, histórico, órdenes de compra y volumen de intercambios. \\
SteamDT & Identifica artículos para priorizar la captura. \\
\acroref{OCR}{OCR}, \acroref{CSV}{CSV} y \acroref{API}{API} & Incorporan capturas y archivos como observaciones. \\
\addlinespace
\multicolumn{2}{l}{\textit{Decisión multiagente}} \\
Scout & Marca artículos según el retorno previsto, la disponibilidad y la señal supervisada. \\
Trader & Propone comprar, vender o mantener. \\
...
\noindent\textbf{Scout.} Scout se encarga de detectar oportunidades. Su observación está orientada a la plataforma, el tipo de publicación, el precio de compra, el retorno previsto, la disponibilidad y la probabilidad supervisada. Su papel no es decidir toda la operación, sino marcar qué artículos parecen merecer atención dentro del episodio.

\noindent\textbf{Trader.} Trader decide la acción operativa principal: mantener, comprar una unidad o solicitar la venta de una posición desbloqueada. Para ello considera la señal de Scout, la información de compra y venta prevista, el retorno calculado, la probabilidad supervisada y las posiciones del mismo artículo ya presentes en la cartera.

\noindent\textbf{Portfolio.} Portfolio controla la coherencia global de la cartera. Su observación incorpora saldo, capital bloqueado, exposición prevista, posiciones que pueden venderse y restricciones de riesgo. Este agente puede rechazar decisiones que parezcan favorables de forma aislada si exceden los límites configurados.
