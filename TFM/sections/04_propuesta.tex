...
Partiendo de la formulación general presentada en la Sección~\ref{sec:estado-arte-marl}, el problema de este trabajo se modela como un sistema \acroref{MARL}{MARL} con observabilidad parcial. En un proceso de decisión de Markov parcialmente observable, conocido como \acroref{POMDP}{POMDP} (\textit{Partial Observability Markov Decision Process}), el entorno mantiene un estado real, pero el agente solo recibe una observación incompleta de ese estado \cite{kaelbling1998planning}. En este trabajo se utiliza esa idea en un contexto multiagente: existe un estado global de mercado y cartera, pero cada agente recibe solo la parte de información que necesita para cumplir su responsabilidad \cite{wooldridge2009multiagent,busoniu2008comprehensive}.
...
\begin{itemize}
  ...
  \item \textbf{Observaciones locales.} Cada agente recibe una observación local $o_t^i\in\mathcal{O}^i$, es decir, una selección del estado global adaptada a su responsabilidad. Scout observa principalmente información del candidato, actividad de mercado y señal supervisada. Trader observa esas señales junto con información de salida, saldo disponible y posiciones del mismo artículo. Portfolio observa ratios de cartera, avisos de riesgo, número de incumplimientos y posiciones vendibles. Esta distinción evita asumir que todos los agentes conocen toda la información del sistema.

  \item \textbf{Acciones.} El conjunto de acciones se define por rol y contiene siete acciones disponibles repartidas entre los tres agentes:
  \begin{itemize}
    \item \textbf{Scout:} \textit{ignorar candidato} o \textit{marcar candidato}.
    \item \textbf{Trader:} \textit{mantener cartera}, \textit{comprar una unidad} o \textit{vender una unidad}.
    \item \textbf{Portfolio:} \textit{rechazar propuesta} o \textit{aprobar propuesta}.
  \end{itemize}
  En cada instante, cada agente elige una de sus acciones disponibles. La combinación de las tres decisiones forma la \textbf{\glosref{accion-conjunta}{acción conjunta}} $\mathbf{a}_t=(a_t^{S},a_t^{T},a_t^{P})$. Como hay dos acciones de Scout, tres de Trader y dos de Portfolio, existen hasta $2\cdot3\cdot2=12$ combinaciones posibles. No todas producen una operación económica. Por ejemplo, una compra simulada solo se intenta cuando Scout marca el candidato, Trader propone comprar una unidad y Portfolio aprueba la propuesta. Una venta simulada solo se intenta cuando Trader propone vender una unidad, Portfolio aprueba la propuesta y existe una posición desbloqueada del mismo artículo. En el resto de combinaciones, la cartera se mantiene o se registra una decisión no ejecutada.

  \item \textbf{Simulador y transición.} Las acciones no las devuelve el simulador. Las acciones las generan las políticas de los agentes a partir de sus observaciones locales. El simulador recibe la acción conjunta, comprueba qué caso representa y calcula la transición $P_{\mathrm{sim}}(s_{t+1}\mid s_t,\mathbf{a}_t)$. La transición es la regla que determina el estado resultante $s_{t+1}$ después de aplicar la decisión sobre el estado actual $s_t$.
  \begin{itemize}
    \item \textbf{Compra válida:} si la acción conjunta solicita una compra y la propuesta cumple las restricciones, el estado siguiente reduce el efectivo disponible, abre una nueva posición, registra el coste invertido, actualiza la exposición de cartera y guarda la fecha a partir de la cual la unidad podrá venderse.
    \item \textbf{Venta válida:} si la acción conjunta solicita una venta, Portfolio la aprueba y existe una posición desbloqueada del mismo artículo, el estado siguiente cierra esa posición, aplica comisiones, incorpora el importe neto al efectivo disponible y actualiza el beneficio de la operación.
    \item \textbf{Decisión no ejecutada o inválida:} si la acción conjunta no solicita una operación completa, si no hay saldo suficiente, si no existe posición vendible o si se incumple una restricción de riesgo, el estado siguiente conserva la cartera económica sin abrir ni cerrar posiciones. Aun así, el simulador registra el motivo de no ejecución para que la recompensa pueda distinguir entre esperar voluntariamente y proponer una operación imposible.
  \end{itemize}
...
El factor de descuento $\gamma$ mantiene el significado presentado en la Sección~\ref{sec:estado-arte-rl}. En este problema es relevante porque la recompensa económica suele aparecer después de la compra, cuando una venta posterior cierra la posición. Un valor de $\gamma$ alto permite que el entrenamiento relacione decisiones de compra con recompensas futuras, mientras que un valor bajo daría demasiado peso a señales inmediatas \cite{sutton2018reinforcement}. La implementación experimental utiliza $\gamma=0{,}99$, como se detalla en los parámetros de entrenamiento del Anexo~A.
...
\begin{algorithmic}[1]
\Require Artículo candidato $c_t$, estado global $s_t$ y observaciones locales $o_t^S$, $o_t^T$ y $o_t^P$
...
En el Algoritmo~\ref{alg:decision-cooperativa}, $c_t$ representa el artículo considerado en el instante $t$. Las funciones $\pi_S$, $\pi_T$ y $\pi_P$ son las políticas de Scout, Trader y Portfolio. Cada política recibe su observación local y produce una acción. El simulador no sustituye esas políticas: únicamente interpreta $\mathbf{a}_t$, aplica las reglas de transición y calcula las recompensas que se explican en la Sección~\ref{sec:funcion-recompensa}.
