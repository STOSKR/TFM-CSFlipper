...
\midrule
\texttt{packages/domain} y \texttt{contracts} & Identificadores canónicos, tipos de datos y validaciones compartidas. \\
\texttt{packages/persistence} & Conexión a PostgreSQL y repositorios de artículos, histórico y señales. \\
\texttt{packages/simulation} & Comisiones, posiciones, bloqueos, riesgo y métricas de cartera. \\
\texttt{packages/prediction} & Construcción de variables, entrenamiento, calibración e inferencia supervisada. \\
\texttt{packages/marl} & Entorno paralelo, recompensas, adaptador PettingZoo y entrenamiento PPO. \\
\texttt{packages/vision} & Preprocesado de imágenes, OCR y parsing de observaciones. \\
\texttt{apps/cli} y \texttt{apps/web} & Comandos reproducibles y consola local de consulta. \\
\bottomrule
\end{tabularx}
\caption{Distribución funcional del repositorio.}
...
\begin{algorithm}[H]
\caption{Trabajo de scraping trazable}
\label{alg:scraping-trazable}
\begin{algorithmic}[1]
\Require Lista de candidatos, conectores activos y configuración de trabajo
...
\bottomrule
\end{tabularx}
\caption{Vistas principales de la consola local.}
...
\begin{algorithm}[H]
\caption{Ejecución reproducible de un experimento}
