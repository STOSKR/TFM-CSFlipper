...
\State $a_t^{S} \gets \pi_S(o_t^{S})$ \Comment{Scout marca o ignora}
\State $a_t^{T} \gets \pi_T(o_t^{T},m_t^{T})$ \Comment{Trader mantiene o compra}
\State $a_t^{P} \gets \pi_P(o_t^{P},m_t^{P})$ \Comment{Portfolio rechaza o aprueba}
