...
Esta sección reúne los resultados obtenidos durante la construcción del sistema. Se presentan primero las comprobaciones sobre reglas económicas y modelos supervisados, después la evolución de los conjuntos de compraventa y, por último, las pruebas técnicas, el \acroref{OCR}{OCR} y el entorno multiagente. Esta ordenación mantiene separadas tres evidencias distintas: que los cálculos económicos son reproducibles, que la señal supervisada puede estimarse sobre datos históricos y que el simulador \acroref{MARL}{MARL} ejecuta decisiones de cartera.

\subsection{Reglas económicas y modelo supervisado}

\textbf{Migración de reglas económicas.}
...
\textbf{Resultados del modelo supervisado.}

La fase supervisada conserva una exploración de dirección Steam con 47.467 filas de entrenamiento, 19.252 de validación y 18.412 de prueba. En este contexto, la dirección indica si el precio futuro de Steam se mueve de forma favorable respecto al precio observado, no si una compraventa entre plataformas produce beneficio neto. Por tanto, esta exploración sirve para comprobar que el flujo de variables, entrenamiento, calibración y selección por umbral funciona con una muestra de mayor tamaño, pero no se utiliza para aprobar compras. El protocolo principal de este capítulo es el corte BUFF--Steam que empieza en diciembre de 2025, porque ese corte sí representa una operación de compra y venta con comisiones.
...
El dataset de compraventa no ha aparecido en una única ejecución. Se han construido varios cortes porque cada uno ha revelado una limitación diferente del histórico disponible. El primer \texttt{trading\_profit\_v1} ha reunido 2.331 filas y 50 artículos. Ha servido para comprobar el constructor de etiquetas, las comisiones y la persistencia de características, pero la dirección analizada solo ha conservado un caso positivo en validación y otro en prueba. Por ese motivo sus métricas no se comunican como resultado de un modelo.

Después se han corregido el parser de \acroref{BUFF}{BUFF}, la normalización de moneda y el backfill de puntos históricos. Con estas correcciones se han ejecutado tres cortes BUFF--Steam desde diciembre de 2025. La Tabla~\ref{tab:cortes-datos} deja visibles tanto las muestras que permiten una exploración como las que se descartan. Esta distinción es importante: un dataset grande no es suficiente si sus bloques de validación solo contienen una clase.
...
\textbf{Ablación de histórico y aumentación.}
...
Con la base de datos conectada se ha repetido la construcción del dataset de BUFF a Steam el 11 de agosto de 2026. Se ha empleado la ventana desde diciembre de 2025, un horizonte de ocho días, una purga de ocho días y una prueba acotada a julio de 2026. El conjunto resultante conserva los 19 artículos ya presentes en el corte de marzo. El conjunto no permite entrenar otro clasificador y sirve para comprobar si la nueva captura ha aportado variedad suficiente de etiquetas.
...
\subsection{Uso operativo de los datos de compraventa y pivotaciones}

El dataset \texttt{trading\_profit\_v1} se ha generado desde \texttt{market\_history\_points} usando precios Steam/\acroref{BUFF}{BUFF}, comisiones y horizonte temporal. La primera versión ha producido 2.331 ejemplos y 50 artículos, pero con desbalance extremo en validación y prueba. Por este motivo, sus métricas no se usan como evidencia de rendimiento operativo. Las correcciones posteriores del parser, la normalización y el backfill desembocan en \texttt{trading\_profit\_v2}, descrito en la Tabla~\ref{tab:trading-v2-particiones}.

El análisis posterior ha mostrado que el histórico de \acroref{BUFF}{BUFF} empieza a estar mejor alineado tras corregir el parser y permitir backfill de puntos históricos. El conjunto actual ya permite entrenar y validar políticas en los tres bloques temporales, pero mantiene una distribución de etiquetas sesgada. Por ello, sus resultados se presentan con su tasa de operaciones rentables y no se interpretan como una garantía de rentabilidad futura.
...
\textbf{Dificultades y pivotaciones.}
...
\begin{table}[htbp]
\centering
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.32\textwidth} r r}
\toprule
