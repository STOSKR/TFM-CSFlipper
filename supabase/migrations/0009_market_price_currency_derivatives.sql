...
set
    price_eur = case
        ...
        when upper(currency) = 'CNY' then metric_value / 8
        else null
    end,
    price_cny = case
        when metric_name not in ('sell_price', 'buy_order_price') then null
        ...
        when upper(currency) = 'EUR' then metric_value * 8
        else null
    end;
